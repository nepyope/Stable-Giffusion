    ...
    def sample(input_ids: jax.Array, attention_mask: jax.Array, guidance: int):
        ...
        def _step(original_latents, idx):
            def _inner(state, i):
                ...
                inp = lax.dynamic_index_in_dim(original_latents, jnp.maximum(idx - ctx, 0), keepdims=True)
            ...
            latents = jax.random.normal(jax.random.PRNGKey(0), (1, 4, 16, 16), jnp.float32)
