...
def main(lr: float = 1e-4, beta1: float = 0.9, beta2: float = 0.99, weight_decay: float = 0.001, eps: float = 1e-12,
    ...
    unet, unet_params = FlaxUNet2DConditionModel.from_pretrained(
       base_model, subfolder="unet", dtype=jnp.float32)
    unet_params = patch_weights(vae_params)
    unet: FlaxAutoencoderKL = unet
    '''
    ...
    def sample(params, batch: Dict[str, Union[np.ndarray, int]], context_half: int):
            inp = jnp.transpose(batch["pixel_values"][:,:context_half,:,:].astype(jnp.float32) / 255, (0, 3, 1, 2))
    ...
    def train_step(state: train_state.TrainState, batch: Dict[str, Union[np.ndarray, int]]):
        ...
        def compute_loss(params):
            ...
            # Only compare the second half of context with VAE output
            dist = out - img[:,context//2:,:,:]
            dist = dist.reshape(-1, context//2, *dist.shape[1:])
    ...
    for epoch in range(100):
        for i, video in tqdm.tqdm(enumerate(data, 1)):
            ...
            if i % sample_interval == 0:
                s_rng, s_mode = to_host(p_sample(state.params, batch, context//2))
                ...
                extra["Samples/Ground Truth"] = wandb.Image(batch["pixel_values"][:,context//2:,:,:].reshape(-1, resolution, 3) / 255)
